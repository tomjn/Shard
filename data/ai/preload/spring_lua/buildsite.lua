...
function CreateBuildsiteFinder(myTeamID)
	Spring.Echo("createbuildsitefinder", myTeamID)
...
local function Initialize()
	for unitDefID, unitDef in pairs(UnitDefs) do
		if (unitDef.speed == 0) then -- not gadget.flags[unitDefID]
...
function widget.FindBuildsite(builderpos, unitDefID, closest, searchradius, minimumdistance)
	...
	local minimumdistanceSq = minimumdistance * minimumdistance
	local vertices = DrawMain()
	local count = vertices.vi

	-- assume builder position is representative for base position
	local x,y,z = builderpos.x, builderpos.y, builderpos.z
	...
	if UnitDefs[unitDefID].speed == 0 and count > 1.5 then
		if closest then
			for i = #vertices, 1, -1 do
				...
				if v.sqdist < minimumdistanceSq or v.sqdist > searchradiusSq then
