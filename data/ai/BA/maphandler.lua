...
function MapHandler:CheckDefenseLocalization(unitName, position)
	...
	--[[
		-- testing repeated table loops
	local units = game:GetUnits()
	local testunit
	for nothing, u in pairs(units) do
		if u ~= nil then
			testunit = u
			break
		end
	...
	for x = 1, 9 do
		for z = 1, 9 do
			...
			pos.x = x + 0.5
			pos.z = z + 0.12345
			table.insert(test, pos)
		end
	end
	local utype = game:GetTypeByName("armpeep")
	local thing
	local success
	for n = 1, 9 do
		local thistest = test
		EchoDebug("LOOP " .. n .. " START")
		for i, p in pairs(thistest) do
			EchoDebug("#" .. i .. " " .. p.x .. " , " .. p.z)
			local on = n + 1
			...
			thing = p
			success = testunit:Build(utype, p)
		end
		EchoDebug("LOOP " .. n .. " END")
		EchoDebug(" ")
	end
